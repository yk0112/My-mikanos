  ...
  void Initialize() {
    ...
    for(int i = 0; i < pci::num_device; i++) {
        if(pci::devices[i].class_code.Match(0x0cu, 0x03u, 0x30u)) {
            ...
            if(0x8086 == pci::ReadVendorId(*xhc_dev)) { // made by intel ?
                break;
            }
        }
    }
    ...
    const uint64_t bsp_local_apic_id = *reinterpret_cast<const uint32_t*>(0xfee00020) >> 24; // 現在のCPUコア番号を取得
    // Capability構造体にMSI割り込みの設定を書き込み
    ...
    // xHCの制御レジスタ(MMIO)のアドレスを取得
    const WithError<uint64_t> xhc_bar = pci::ReadBar(*xhc_dev, 0);
    Log(kDebug, "ReadBar: %s\n", xhc_bar.error.Name());
    const uint64_t xhc_mmio_base = xhc_bar.value & ~static_cast<uint64_t>(0xf); // 下位4ビットは制御フラグのためマスク
    ...
    if(0x8086 == pci::ReadVendorId(*xhc_dev)) { // Intel Panther Point chip用の設定
    ...
    for(int i = 0; i <= xhc.MaxPorts(); i++) {
        ...
        if(port.IsConnected()) { // USBポートに機器が接続されているか
            if(auto err = ConfigurePort(xhc, port)) { // ADL(実引数依存の名前探索)
